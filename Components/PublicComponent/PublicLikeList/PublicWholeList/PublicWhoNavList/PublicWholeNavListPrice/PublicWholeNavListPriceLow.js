...
@observer
export default class PublicWholeNavListPriceLow extends PureComponent{
  ...
  _onEndReached=async()=>{
      page++;
      const json = await fetchJson(`${Api}num=${num}&page=${page}&OrderBy=${this.props.OrderBy}&HighLow=${this.props.HighLow}&table=${NewDaTaoKe.DaTaoKe?'dataoke':'taobao'}&ByDict=${this.props.title}`);
      InteractionManager.runAfterInteractions(()=>{
          this.setState({
