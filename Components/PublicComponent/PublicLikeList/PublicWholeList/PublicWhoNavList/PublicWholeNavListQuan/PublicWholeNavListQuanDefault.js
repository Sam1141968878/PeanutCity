...
export default class PublicWholeNavListQuanDefault extends PureComponent{
  ...
  FetchApi=`${Api}num=${num}&page=${page}&OrderBy=${this.props.OrderBy}&cid=${this.props.title}`
  ...
  _onEndReached=async()=>{
      page++;
      const json = await fetchJson(`${Api}num=${num}&page=${page}&OrderBy=${this.props.OrderBy}&cid=${this.props.title}`);
      InteractionManager.runAfterInteractions(()=>{
          this.setState({
